...
export default function CardList({ card }: { card: CompleteCard[] }) {
  ...
  return (
    <div>
      <Modal open={open} setOpen={setOpen} title={activeCard ? 'Edit Card' : 'Create Card'}>
        <CardForm card={activeCard} addOptimistic={addOptimisticCard} openModal={openModal} closeModal={closeModal} />
      </Modal>
...
const Card = ({ card, openModal }: { card: CompleteCard; openModal: TOpenModal }) => {
...
const EmptyState = ({ openModal }: { openModal: TOpenModal }) => {
  return (
    <div className="text-center">
      <h3 className="mt-2 text-sm font-semibold text-secondary-foreground">No card</h3>
      <p className="mt-1 text-sm text-muted-foreground">Get started by creating a new card.</p>
      <div className="mt-6">
